...
public sealed class GameSyncService : ReactiveObject, IDisposable
{
    ...
    private const uint MaxMessageLength = 1024;
    ...
    public void TeleportToOrigin(string origin) =>
        _ = PostMessage(new Message { Type = MessageType.TeleportToLocation, Content = origin });
    ...
    private async Task ReceiveIncomingMessages()
    {
        ...
        while (_client.State is WebSocketState.Open or WebSocketState.CloseReceived)
        {
            ...
            {
                _logger.Error("Received partial message, which is not supported. Discarding.");
                continue;
            }
    ...
    private void HandleMessage(ReadOnlySpan<byte> messageData)
    {
        ...
        switch (message.Type)
        {
            case MessageType.CurrentPosition:
                ...
                break;
            case MessageType.TargetedEntitiesList:
    ...
    // Corresponds to LumperSyncMessage in C++
    private enum MessageType
    {
        Invalid = 0,
        CurrentPosition = 1,
        TargetedEntitiesList = 2,
