...
public abstract class LumpWriter(Stream output) : BinaryWriter(output)
{
    ...
    {
        // Doesn't seem write to ever mark compressed if empty, since logic might be expected LZMA headers
        // if compressed and we don't want to write those for a totally empty lump.
        if (lump.Empty)
        {
